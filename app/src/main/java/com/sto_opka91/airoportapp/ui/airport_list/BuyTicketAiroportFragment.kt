...
import com.sto_opka91.airoportapp.databinding.FragmentBuyTicketAiroportBinding
import com.sto_opka91.airoportapp.ui.airport_list.StateHolder.AirportListActions
import com.sto_opka91.airoportapp.ui.airport_list.recyclerview.adapter.BuyTicketAdapter
...
class BuyTicketAiroportFragment : Fragment() {
    private var _binding: FragmentBuyTicketAiroportBinding? = null
    ...
    override fun onCreateView(
        ...
        _binding = FragmentBuyTicketAiroportBinding.inflate(inflater, container, false)
    ...
    private fun initPlaneAdapter(): BuyTicketAdapter =
        BuyTicketAdapter(
            ...
            onFavoriteClick = { planeInfo ->
                viewModel.toggleFavorite(planeInfo)
            },
            onBuyClick = { planeInfo ->
                viewModel.onPlaneClick(AirportListActions.SelectPlaneEvent(planeInfo))
                viewModel.onPlaneClick(AirportListActions.NavigateToAddOptionsTicketFlight)
            }
        )
            ...
            viewModel.actions.collect { action ->
                when (action) {
                    is AirportListActions.NavigateToInfoPlane -> {
                        findNavController().navigate(R.id.action_buyTicketAiroportFragment_to_detailInfoPlaneFragment)
                    }
                    is AirportListActions.NavigateToSettings -> {

                        findNavController().navigate(R.id.action_buyTicketAiroportFragment_to_settingsMainFragment)
                    }
                    is AirportListActions.NavigateToAddOptionsTicketFlight -> {

                        findNavController().navigate(R.id.action_buyTicketAiroportFragment_to_buyTicketAddOptionsFragment)
                    }
